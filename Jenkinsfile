...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="happychoi/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/Kevin-Choi-KOR/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/Kevin-Choi-KOR/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://Kevin-Choi-KOR:${GITHUB_ACCESS_TOKEN}@github.com/Kevin-Choi-KOR/guestbook-config.git"
                    '''
                }
            }
        }
